...
export const EditNewRepeatScheduleForm: React.FC<Props> = ({
  ...
  if (academicYear && toBeUpdatedRepeatSchedulePayload && newRepeatSchedules)
    return (
      <div>
        ...
        <Button
          onClick={() => {
            const newToBeUpdatedRepeatSchedules = mutateItemInArray(
              newRepeatSchedules,
              newRepeatSchedules.findIndex(
                (el) => JSON.stringify(el) === JSON.stringify(data)
              ),
              toBeUpdatedRepeatSchedulePayload
            );
            console.log(newToBeUpdatedRepeatSchedules);
            dispatch(setNewRepeatSchedules([...newToBeUpdatedRepeatSchedules]));
            setShow(false);
          }}
          style={{
            padding: "8px",
