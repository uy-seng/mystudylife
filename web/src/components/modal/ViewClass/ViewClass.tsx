          ...
          <div className={css.detail}>
            {data?.schedule.oneOff && (
              <div className={css.row}>
                ...
                <div>
                  {`${moment(data.schedule.oneOff.startTime, "hh:mm:mm").format(
                    "h:mm A"
                  )} - ${moment(
            ...
            {data?.schedule.repeat?.rotationWeek && (
              <div className={css.rotationWeek}>
                <div className={ctx(css.weekNumber, css.levelOne)}>
                  <span>{`Week ${data?.schedule.repeat?.rotationWeek}`}</span>
                  <div />
                </div>
                <div className={ctx(css.weekDetail, css.levelTwo)}>
                  <div>{data?.schedule.repeat?.repeatDays.join(", ")}</div>
                  <div>
                    {`${moment(
                      data.schedule.repeat.startTime,
                      "hh:mm:mm"
                    ).format("h:mm A")} - ${moment(
                      data.schedule.repeat.endTime,
                      "hh:mm:mm"
                    ).format("h:mm A")}`}
                  </div>
                </div>
