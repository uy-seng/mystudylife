var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Subject, Task, User } from "..";
import { Column, Entity, JoinColumn, ManyToOne, OneToMany, OneToOne, PrimaryGeneratedColumn, } from "typeorm";
import { Field, ObjectType } from "type-graphql";
var Exam = (function () {
    function Exam() {
    }
    __decorate([
        PrimaryGeneratedColumn("uuid"),
        Field(function () { return String; }),
        __metadata("design:type", String)
    ], Exam.prototype, "id", void 0);
    __decorate([
        Column(),
        Field(function () { return Boolean; }),
        __metadata("design:type", Boolean)
    ], Exam.prototype, "resit", void 0);
    __decorate([
        Column(),
        Field(function () { return String; }),
        __metadata("design:type", String)
    ], Exam.prototype, "module", void 0);
    __decorate([
        Column("date"),
        Field(function () { return String; }),
        __metadata("design:type", String)
    ], Exam.prototype, "date", void 0);
    __decorate([
        Column("time"),
        Field(function () { return String; }),
        __metadata("design:type", String)
    ], Exam.prototype, "startTime", void 0);
    __decorate([
        Column("time"),
        Field(function () { return String; }),
        __metadata("design:type", String)
    ], Exam.prototype, "endTime", void 0);
    __decorate([
        Column(),
        Field(function () { return String; }),
        __metadata("design:type", String)
    ], Exam.prototype, "seat", void 0);
    __decorate([
        Column(),
        Field(function () { return String; }),
        __metadata("design:type", String)
    ], Exam.prototype, "room", void 0);
    __decorate([
        OneToOne(function () { return Subject; }),
        JoinColumn(),
        Field(function () { return Subject; }),
        __metadata("design:type", Subject)
    ], Exam.prototype, "subject", void 0);
    __decorate([
        OneToMany(function () { return Task; }, function (task) { return task.exam; }),
        __metadata("design:type", Array)
    ], Exam.prototype, "tasks", void 0);
    __decorate([
        ManyToOne(function () { return User; }, function (user) { return user.exams; }),
        __metadata("design:type", User)
    ], Exam.prototype, "user", void 0);
    Exam = __decorate([
        Entity("exams"),
        ObjectType()
    ], Exam);
    return Exam;
}());
export { Exam };
//# sourceMappingURL=Exam.js.map