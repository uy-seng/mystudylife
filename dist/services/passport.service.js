...
class PassportService {
  ...
  initGoogleOAuthStrategy() {
    this.passport.use(
      new passport_google_oauth20_1.default.Strategy(
        {
        ...
        async (_accessToken, _refreshToken, profile, done) => {
          const user = await await (0, typeorm_1.getConnection)(
          ...
          if (user) {
            ...
            const newUserProvider = await (0, typeorm_1.getConnection)(
              process.env.NODE_ENV
            )
            ...
            const newUser = await (0, typeorm_1.getConnection)(
              process.env.NODE_ENV
            )
              .getRepository(entity_1.User)
              .create({
                email: profile.emails[0].value,
                username: profile.displayName.concat(
                  (0, uuid_1.v4)().split("-")[0]
                ),
  ...
  initFacebookOAuthStrategy() {
    this.passport.use(
      new passport_facebook_1.default.Strategy(
        {
